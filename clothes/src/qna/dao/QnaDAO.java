...
private static QnaDAO instance = new QnaDAO();
	...
	public int insertArticle(QnaVO article){
        ...
        try {
            ...
            sql = "insert into qna(clothes_id,clothes_title,qna_writer,qna_content,";
		    sql += "group_id,qora,reply,reg_date) values(?,?,?,?,?,?,?,?)";
            pstmt = conn.prepareStatement(sql);
            pstmt.setInt(1, article.getClothes_id());
            pstmt.setString(2, article.getClothes_title());
	...
	public int getArticleCount(int clothes_id){
        Connection conn = null;
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        int x=0;
        ...
        try {
            ...
            pstmt = conn.prepareStatement("select count(*) from qna where clothes_id = "+clothes_id);
            rs = pstmt.executeQuery();
        ...
        try {
            ...
            if (rs.next()) {//ResultSet이 레코드를 가짐
                ...
                do{
				  ...
				  article.setClothes_id(rs.getInt("clothes_id"));
				  article.setClothes_title(rs.getString("clothes_title"));
	...
	public List<QnaVO> getArticles(int count, int clothes_id){
        Connection conn = null;
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        List<QnaVO> articleList=null;//글목록을 저장하는 객체
        try {
            ...
            pstmt = conn.prepareStatement(
            	"select * from qna where clothes_id="+clothes_id+" order by group_id desc, qora asc");
